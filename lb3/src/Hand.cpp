...
void Hand::setSpellsFromNames(const std::vector<std::string>& spellNames) {
    for (Spell* spell : spells) {
        delete spell;
    }
    spells.clear();
    ...
    for (const auto& name : spellNames) {
        if (name == "Огненная стрела") {
            spells.push_back(new DamageSpell());
        } else if (name == "Взрывная волна") {
            spells.push_back(new AreaSpell());
        }
    }
}
